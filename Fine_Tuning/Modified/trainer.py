...
class Trainer:
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for _ in range(total_updates):
                ...
                for i, inputs in enumerate(batch_samples):
                    ...
                    with context():
                        tr_loss_step, score = self.training_step(model, inputs, num_items_in_batch)
                    ...
                    if do_sync_step:
                        ...
                        self._maybe_log_save_evaluate(tr_loss, score, grad_norm, model, trial, epoch, ignore_keys_for_eval)
                    else:
    ...
    def training_step(
        ...
        if self.use_apex:
            ...
            if num_items_in_batch is None:
                return (loss.detach() / self.args.gradient_accumulation_steps, score)
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            losses, scores, logits, labels = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            main_input_name = getattr(self.model, "main_input_name", "input_ids")
            inputs_decode = (
            ...
            if self.args.batch_eval_metrics:
                if self.compute_metrics is not None and logits is not None and labels is not None:
                    ...
                    batch_kwargs["scores"] = losses if "score" in args.include_for_metrics else None
        ...
        if (
            ...
            eval_set_kwargs["scores"] = all_scores if "loss" in args.include_for_metrics else None
        ...
        if isinstance(all_scores, list) and all_scores:
            metrics[f"{metric_key_prefix}_f1_score"] = mean(all_scores)
