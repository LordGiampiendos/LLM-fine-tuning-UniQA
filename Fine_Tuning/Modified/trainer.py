...
# Preprocessing Method for Bert Score Evaluation
def process_texts(predicted_texts, reference_texts, word):
    ...
    for predicted in predicted_texts:
        if word in predicted:
            indice = predicted.find(word)
            processed_predicted_texts.append(predicted[indice + len(word):])
        else:
            processed_predicted_texts.append(predicted)

    for reference in reference_texts:
        if word in reference:
            indice = reference.find(word)
            processed_reference_texts.append(reference[indice + len(word):])
        else:
...
class Trainer:
    ...
    def __init__(
        ...
        if model.config._name_or_path == "meta-llama/Llama-3.2-3B-Instruct":     
            ...
            word = "### Risposta"
        elif model.config._name_or_path == "Qwen/Qwen2-7B-Instruct":
            word = "<|im_start|>assistant"
        elif model.config._name_or_path == "occiglot/occiglot-7b-eu5-instruct":
            word = "<|im_start|> assistant"
        elif model.config._name_or_path == "google/gemma-2-9b-it":
            word = "<start_of_turn>model\n<end_of_turn>\n<start_of_turn>model"
        elif model.config._name_or_path == "meta-llama/Llama-3.3-70B-Instruct":     
            word = "assistant<|end_header_id|>"
        elif model.config._name_or_path == "mistralai/Mixtral-8x7B-Instruct-v0.1":     
            word = "[/INST]"
        else:
            word = "<|end|><|assistant|>"
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for _ in range(total_updates):
                ...
                for i, inputs in enumerate(batch_samples):
                    ...
                    with context():
                        tr_loss_step, score = self.training_step(model, inputs, num_items_in_batch) # Score
                    ...
                    if do_sync_step:
                        ...
                        self._maybe_log_save_evaluate(tr_loss, score, grad_norm, model, trial, epoch, ignore_keys_for_eval) # Score
                    else:
        ...
        metrics["bert_score"] = score # Let's add the bert score to the metrics
    ...
    def _maybe_log_save_evaluate(self, tr_loss, score, grad_norm, model, trial, epoch, ignore_keys_for_eval):
        if self.control.should_log and self.state.global_step > self._globalstep_last_logged:
            ...
            logs["f1_score"] = score # Added f1_score
    ...
    def training_step(
        ...
        # Logits and Labels Extraction 
        logits = outputs["logits"]
        labels = labels.cpu()
        predicted_tokens = torch.argmax(logits, dim=-1).cpu()
        
        # Logits and Labels Filtering
        filtered_labels = np.where(labels != -100, labels, self.tokenizer.pad_token_id)
        filtered_predictions = np.where(predicted_tokens != -100, predicted_tokens, self.tokenizer.pad_token_id)
        
        # Logits and Labels Decoding
        predicted_texts = [self.tokenizer.decode(tokens, skip_special_tokens=False, clean_up_tokenization_spaces=True) for tokens in filtered_predictions] 
        reference_texts = [self.tokenizer.decode(tokens, skip_special_tokens=False, clean_up_tokenization_spaces=True) for tokens in filtered_labels]
        
        # Logits and Labels Preproccessing
        predicted_texts, reference_texts = process_texts(predicted_texts=predicted_texts, reference_texts=reference_texts, word=word)

        # Bert Score Evaluation
        bert_score = bertscore.compute(predictions=predicted_texts, references=reference_texts, lang="it", device="cpu")
    
        score = round(mean(bert_score['f1']), 2)
        ...
        if self.use_apex:
            ...
            if num_items_in_batch is None:
                return (loss.detach() / self.args.gradient_accumulation_steps, score) # Score
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            losses, scores, logits, labels = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys) # Score
            main_input_name = getattr(self.model, "main_input_name", "input_ids")
            inputs_decode = (
            ...
            if self.args.batch_eval_metrics:
                if self.compute_metrics is not None and logits is not None and labels is not None:
                    ...
                    batch_kwargs["scores"] = losses if "score" in args.include_for_metrics else None # Score
                ...
                del losses, scores, logits, labels, inputs # Score
                torch.cuda.empty_cache()
            ...
            # Gather all tensors and put them back on the CPU if we have done enough accumulation steps.
                ...
                del losses, scores, logits, labels, inputs
                torch.cuda.empty_cache()
        ...
        if (
            ...
            eval_set_kwargs["scores"] = all_scores if "loss" in args.include_for_metrics else None # Score
        ...
        if isinstance(all_scores, list) and all_scores:
            metrics[f"{metric_key_prefix}_f1_score"] = mean(all_scores) # Score
    ...
    def prediction_step(
        ...
        if prediction_loss_only:
            ...
            labels = labels.cpu()
            predicted_tokens = predicted_tokens = torch.argmax(logits, dim=-1).cpu()
            ...
            del logits, labels
            scores = round(mean(bert_score['f1']), 2)
            
            return(loss, scores, None, None) # Score
